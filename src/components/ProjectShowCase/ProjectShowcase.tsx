...
function ProjectShowcase({
    ...
    const { setInteractionMode } = useInteraction();
    ...
    return (
        ...
        >
            ...
            <motion.img
                ...
                src={activeImage}
                alt={project.title}
                className="absolute inset-0 h-full w-full object-cover blur-[8px] scale-105"
            />
            ...
            {/* =========================================
                MOBILE & TABLET VIEW (BOTTOM SHEET DRAWER) 
            =========================================== */}
            <div className="xl:hidden relative z-10 h-full w-full flex flex-col">
                {/* Mobile Text Layer (Z-Index 10) */}
                <div className="flex-1 px-8 pt-16 pointer-events-auto">
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <span className="text-[10px] uppercase tracking-[0.2em] text-[#efc07b]">
                            Featured Project
                        </span>
                    </div>

                    {/* RESTORED FONT LAYOUT */}
                    <h1 className="text-4xl font-bold tracking-tight leading-none text-white mb-2">
                        {project.title}
                    </h1>
                    <p className="text-lg font-medium text-orange-300/90 mb-6">
                    ...
                    <div className="flex flex-wrap gap-2 mb-6">
                        {project.stack?.map((techKey) => {
                            ...
                            return (
                                <div key={techKey} title={tech?.name || techKey} className="flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-white/[0.04] backdrop-blur-xl">
                                    {tech?.icon ? (
                                        <img src={tech.icon} alt={tech.name} className="h-5 w-5 object-contain opacity-90" />
                                    ) : (
                                        <span className="text-[9px] font-medium text-white/70">{techKey}</span>
                                    )}
                                </div>
                            );
                        })}
                    ...
                    <p className="text-sm leading-relaxed text-white/70 mb-8 max-w-[400px]">
                        {description}
                    </p>

                    <div className="flex gap-3">
                        <button className="flex items-center justify-center h-12 w-12 rounded-full bg-orange-300 text-black transition-transform active:scale-95">
                        ...
                        <button className="flex-1 h-12 rounded-full border border-white/10 bg-white/5 text-sm font-semibold text-white transition-all active:bg-white/10">
                            Contact Me
                        </button>
                    </div>
                </div>

                {/* Draggable Bottom Sheet Drawer (Z-Index 20) */}
                <motion.div
                    drag="y"
                    dragConstraints={{ top: 0, bottom: 0 }}
                    dragElastic={0.15}
                    onDragEnd={(_, info) => {
                        if (info.offset.y < -30 || info.velocity.y < -300) setIsDrawerOpen(true);
                        else if (info.offset.y > 30 || info.velocity.y > 300) setIsDrawerOpen(false);
                    }}
                    variants={{
                        open: { y: 0 },
                        closed: { y: "calc(100% - 90px)" }
                    }}
                    initial="closed"
                ...
                >
                    <div
                        ...
                        className="h-[90px] w-full flex flex-col items-center justify-center cursor-pointer shrink-0"
                    >
                        <motion.div
                            animate={{ y: isDrawerOpen ? 0 : [0, -4, 0] }}
                            transition={{ repeat: isDrawerOpen ? 0 : Infinity, duration: 2, ease: "easeInOut" }}
                            className="text-white/50 mb-2"
                        >
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={isDrawerOpen ? "rotate-180 transition-transform" : "transition-transform"}>
                                <path d="m18 15-6-6-6 6" />
                            </svg>
                        </motion.div>
                        <span className="text-[10px] uppercase tracking-widest text-white/40 font-medium">
                            {isDrawerOpen ? "Close Gallery" : "View Gallery"}
                        </span>
                    </div>

                    <div className="flex-1 px-6 pb-6 flex flex-col min-h-0">
                        <div className="flex-1 relative rounded-[32px] overflow-hidden border border-white/10 bg-black/40 mb-4 min-h-0">
                            <img
                                src={activeImage}
                                alt={project.title}
                        ...
                        <motion.div
                            ...
                            onPanEnd={(_, info) => {
                                if (Math.abs(info.offset.x) > Math.abs(info.offset.y)) {
                                    if (info.offset.x < -30) setRotationIndex(prev => prev + 1);
                                    if (info.offset.x > 30) setRotationIndex(prev => prev - 1);
                                }
                            }}
                        >
                                ...
                                {[-2, -1, 0, 1, 2].map((offset) => {
                                    ...
                                    return (
                                        <motion.div
                                            ...
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                setRotationIndex(rotationIndex + offset);
                                            }}
            ...
            <div className="hidden xl:flex relative z-10 h-full w-full justify-between px-24 py-16">
                    ...
                    <div className="flex-1 flex flex-col justify-start pt-[6vh]">
                        ...
                        <div className="mb-8 flex flex-wrap gap-3">
                            {project.stack?.map((techKey) => {
                                const tech = techStack[techKey];
                                return (
                                    <div key={techKey} title={tech?.name || techKey} className="flex h-12 w-12 items-center justify-center rounded-full border border-white/10 bg-white/[0.04] backdrop-blur-xl transition-all duration-300 hover:scale-105 hover:bg-white/[0.07]">
                                        {tech?.icon ? (
                                            <img src={tech.icon} alt={tech.name} className="h-6 w-6 object-contain opacity-90" />
                                        ) : (
                                            <span className="text-[10px] font-medium text-white/70">{techKey}</span>
                                        )}
                                    </div>
                                );
                            })}
                ...
                <section className="flex flex-1 items-center justify-start pl-15 pt-[6vh]">
                    <div className="relative h-[65vh] max-h-[580px] w-[48vw] 2xl:w-[55vw] overflow-hidden rounded-[34px] border border-white/10 bg-black/20 backdrop-blur-xl">
                        <img
                            src={activeImage}
                            alt={project.title}
                ...
                {/* RIGHT PANEL (Vertical Revolver Stage) */}
                <section className="relative flex h-full w-[400px] items-center justify-end">
                    <motion.div
                        ...
                        onPanEnd={(_, info) => {
                            if (info.offset.y < -30 || info.velocity.y < -200) setRotationIndex((prev) => prev + 1);
                            else if (info.offset.y > 30 || info.velocity.y > 200) setRotationIndex((prev) => prev - 1);
                        }}
                    >
                        <AnimatePresence mode="popLayout">
                            {[-2, -1, 0, 1, 2].map((offset) => {
                                ...
                                return (
                                    <motion.div
                                        key={absoluteIndex}
                                        onClick={() => setRotationIndex(rotationIndex + offset)}
