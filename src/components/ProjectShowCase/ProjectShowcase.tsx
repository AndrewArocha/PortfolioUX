...
function ProjectShowcase({ project, onClose }: ProjectShowcaseProps) {
  ...
  const description =
    project.description || "Project description placeholder text goes here...";
  const gallery =
    project.gallery && project.gallery.length > 0
      ? project.gallery
      : [project.image];
  const activeGalleryIndex =
    ((rotationIndex % gallery.length) + gallery.length) % gallery.length;
  ...
  return (
    ...
    >
      ...
      <div className="xl:hidden relative z-10 h-full w-full flex flex-col">
        {/* standard stopPropagation to protect text scroll without killing physics */}
        <div
          className="flex-1 px-8 pt-16 pointer-events-auto"
          onPointerDown={(e) => e.stopPropagation()}
        >
            ...
            {project.stack?.map((techKey: keyof typeof techStack) => {
              ...
              return (
                <div
                  ...
                  className="flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-white/4 backdrop-blur-xl"
                >
                  {tech?.icon ? (
                    <img
                      src={tech.icon}
          ...
          <p className="text-sm leading-relaxed text-white/70 mb-8 max-w-100">
            {description}
          </p>
        ...
        <motion.div
          drag="y"
          dragDirectionLock={true}
          // THE FIX: Pixel-based constraints for 1:1 tactile finger tracking
          dragConstraints={
            isDrawerOpen
              ? { top: 0, bottom: window.innerHeight * 0.85 - 90 }
              : { top: -(window.innerHeight * 0.85 - 90), bottom: 0 }
          }
          dragElastic={0.2}
          whileDrag={{ cursor: "grabbing" }}
          onDragEnd={(_, info) => {
            // Logic updated to check direction context
            if (!isDrawerOpen) {
          ...
          variants={{
            open: { y: 0 },
            closed: { y: window.innerHeight * 0.85 - 90 }, // Replaced "calc()" with pixels
          }}
          initial="closed"
        ...
        >
          <div
            ...
            className="h-22.5 w-full flex flex-col items-center justify-center cursor-pointer shrink-0"
          >
            <motion.div
              animate={{ y: isDrawerOpen ? 0 : [0, -4, 0] }}
              transition={{
                repeat: isDrawerOpen ? 0 : Infinity,
                duration: 2,
                ease: "easeInOut",
              }}
          ...
          <div className="flex-1 px-6 pb-6 flex flex-col min-h-0">
            ...
            <motion.div
              className="h-25 relative w-full flex justify-center items-center shrink-0 touch-none"
              onPanEnd={(_, info) => {
            ...
            >
              <AnimatePresence mode="popLayout">
                {[-2, -1, 0, 1, 2].map((offset) => {
                  ...
                  const galleryIdx =
                    ((absoluteIndex % gallery.length) + gallery.length) %
                    gallery.length;
                  ...
                  return (
                    <motion.div
                      ...
                      initial={{
                        x: (offset + Math.sign(offset) * 0.5) * 80,
      ...
      <div className="hidden xl:flex relative z-10 h-full w-full justify-between px-24 py-16">
        <section className="w-[38%] 2xl:w-[45%] h-full flex flex-col">
          ...
          <div className="flex-1 flex flex-col justify-start pt-[6vh]">
              ...
              {project.stack?.map((techKey: keyof typeof techStack) => {
                ...
                return (
                  <div
                    key={techKey}
                    title={tech?.name || techKey}
                    className="flex h-12 w-12 items-center justify-center rounded-full border border-white/10 bg-white/4 backdrop-blur-xl transition-all duration-300 hover:scale-105 hover:bg-white/[0.07]"
                  >
                    {tech?.icon ? (
                      <img
                        src={tech.icon}
            ...
            <p className="max-w-130 text-base leading-relaxed text-white/70">
              {description}
            </p>
        ...
        <section className="flex flex-1 items-center justify-start pl-15 pt-[6vh]">
          <div className="relative h-[65vh] max-h-145 w-[48vw] 2xl:w-[55vw] overflow-hidden rounded-[34px] border border-white/10 bg-black/20 backdrop-blur-xl pointer-events-none">
            <img
              src={activeImage}
              alt={project.title}
        ...
        <section className="relative flex h-full w-100 items-center justify-end">
          <motion.div
            className="relative h-full w-full flex justify-end items-center touch-none"
            onPointerDown={(e) => e.stopPropagation()} // Protects from accidental root swipe on desktop
            onPanEnd={(_, info) => {
              ...
              {[-2, -1, 0, 1, 2].map((offset) => {
                ...
                return (
                  <motion.div
                    ...
                    initial={{
                      y: (offset + Math.sign(offset) * 0.5) * 136,
                      x: Math.abs(offset) * 18 + 20,
                      opacity: 0,
                      scale: 0.6,
                    }}
                    animate={{
                      y: offset * 136,
                      x: Math.abs(offset) * 28,
                      scale: isCenter ? 1 : 0.75 - Math.abs(offset) * 0.1,
                      opacity: isCenter ? 1 : 0.5 - Math.abs(offset) * 0.2,
                    }}
                    exit={{
                      y: (offset + Math.sign(offset) * 0.5) * 136,
                      x: Math.abs(offset) * 28 + 20,
                      opacity: 0,
                      scale: 0.6,
                    }}
